...
#-*- coding: utf-8 -*-
# File Name: main.py
# Created Time: 2022/10/31 14:22:42

# 将最原始的数据进行分词,然后保存到 train_data_fenci.txt
...
def cutWord(train_file, test_file):
    ...
    for single_query in train_data:
        ...
        ID = single_query_list.pop(0)  # 读取不要，进行剔除
        ID_dict[ID] = ID
        ...
        for j, sentence in enumerate(single_query_list):
            ...
            if(Age_dict[ID]!='0'and Gender_dict[ID]!='0' and Education_dict[ID]!=0):
                ...
                # print(stop)
                print('train_processing %d in %d' % (j, train_data_count))
...
def main():
    train_file = 'user_tag_query.2W.TRAIN'
    test_file = 'user_tag_query.2W.TEST'
